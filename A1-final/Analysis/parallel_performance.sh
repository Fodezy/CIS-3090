#!/usr/bin/env bash
set -Eeuo pipefail
mkdir -p Analysis/outputs/parallel/threadScalling parallel/taskScalling
...
for FILE in "$INPUTDIR"/*.txt; do 
    ...
    OUT="Analysis/outputs/parallel/threadScalling/${FNAMESOUT[iter]}.csv"
    : > "$OUT"
...
for FILE in "$INPUTDIR"/*.txt; do 
    ...
    OUT="Analysis/outputs/parallel/taskScalling/${FNAMESOUT[iter]}.csv"
    : > "$OUT"
    echo "mode,threads,tasks,iters_per_task,total_iters,run_id,elapsed_ms" >> "$OUT"
    echo "Found test input file: $FILE Starting 30 iterations now..." 
